...
## Hatırlatma e-postası (re-engagement)
- **Amaç:** bir haftadır uğramayan kullanıcıya "Languruu'ya bir şans daha?" maili. Modül `backend/src/reengagement/` (servis + public controller + cron); sorgular `UserRepository`'de, gövde `MailService.sendReengagementEmail`'de.
- **Uygunluk şartları tek yerde:** `UserRepository.reengagementEligible()` — `deleted_at IS NULL`, ban yok, `email_verified`, `reengagement_opt_out = false`, `COALESCE(last_seen_at, created_at) < now() - 7 gün` ve `reengaged_at` ya boş ya 30 günden eski. Sayaç (`countInactive...`) ile listeyi (`findInactive...`) aynı fragmanı kullanır; ayrışırlarsa panel "12 kişi" der, gönder deyince 30 kişiye gider.
- **`last_seen_at` yerine `COALESCE(last_seen_at, created_at)`**: presence sütunu eklenmeden önce kaydolup hiç dönmemiş kullanıcıların `last_seen_at`'i `NULL`; ham `<` karşılaştırması onları tamamen eleyip tam da hedef kitleyi dışarıda bırakırdı.
- **Cooldown `reengaged_at` sütunudur** (`REENGAGEMENT_COOLDOWN_DAYS` = 30). Cron her gün çalıştığı için bu damga olmadan aynı kişiye her sabah mail giderdi. Damga yalnızca **gönderim başarılıysa** basılır: `MailService.send` artık `boolean` döner (Resend'in `{ error }` gövdesi de başarısızlık sayılır), `runBatch` sadece başarılı id'leri `markReengaged`'e verir.
- Cron `ReengagementTasks`, her gün **10:00 UTC** (TR 13:00). Tek turda en fazla `REENGAGEMENT_BATCH_LIMIT` (200) kişi, aralarında 600 ms bekleme — Resend'in saniyelik istek sınırına takılmamak için. Kalanlar ertesi gün gider.
- **Admin panelinden iki yol** (`app/(dashboard)/admin/AdminReengagement.tsx`): "Hepsine gönder" (`POST /admin/reengagement/run`, `useConfirm` ile onay ister — geri alınamaz) ve **tek adrese elle gönderim** (`POST /admin/reengagement/send`, gövde `{ email }`). Uygun kişi sayısı `GET /admin/reengagement`'ten gelir.
- **Elle gönderim kayıtlı olmayan adrese de gider** (davet/test için). Kullanıcı bulunursa isim kişiselleştirilir, abonelik bağlantısı eklenir ve `reengaged_at` damgalanır; bulunamazsa mail yine gider ama abonelik bağlantısı **konmaz** (bağlanacak satır yok). Kullanıcı daha önce abonelikten çıkmışsa uç **400 + mesaj `REENGAGEMENT_OPTED_OUT`** döner — elle gönderim tercihi ezmez.
- **Abonelikten çıkma imzalıdır, oturum istemez:** `_common/utils/unsubscribe-token.ts` → `<userId>.<HMAC-SHA256(userId, JWT_SECRET) base64url ilk 32 karakter>`; doğrulama `timingSafeEqual` ile. Uç `POST /reengagement/unsubscribe` **guard'sız** (mail alıcısının çerezi olmayabilir), dakikada 10 `@Throttle`. Bayrak `users.reengagement_opt_out`; şifre sıfırlama / doğrulama mailleri etkilenmez.
- **Sayfa `/unsubscribe?t=<token>` link tıklamasında değil, butona basınca yazar.** Kurumsal mail tarayıcıları bağlantıları önden açar; GET'te opt-out yapılsaydı kullanıcı hiç tıklamadan listeden düşerdi. Rota `PROTECTED_PREFIXES`'te değil (public), `robots.ts`'te disallow ve sayfa `robots: noindex`.
- `buildEmail` artık `bodyHtml` (çok paragraflı gövde), `preheader`, `unsubscribeLink` ve `showLinkFallback` opsiyonlarını alır; eski iki mail (doğrulama, şifre sıfırlama) `body` alanıyla aynen çalışır. Hatırlatma mailinde "buton çalışmazsa şu linki kopyala" satırı **bilerek kapalı** — hedef sadece `/study`, ham URL gürültü.
- **Yeni sütunlar migration ister:** `users.reengaged_at` (timestamp) + `users.reengagement_opt_out` (boolean, `DEFAULT false`). Varsayılanın `false` olması bilinçli: mevcut herkes otomatik olarak listede kalır.
